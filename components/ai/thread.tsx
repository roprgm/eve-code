...
function Thread({ children }: PropsWithChildren) {
  return (
    <MessageScroller.Provider autoScroll>
      <MessageScroller.Root className="relative min-h-0 flex-1 overflow-hidden">
        <MessageScroller.Viewport className="app-scrollbar scroll-fade size-full overflow-y-auto overscroll-contain scrollbar-gutter-stable">
          <MessageScroller.Content className="flex h-max min-h-full flex-col px-4 py-5 sm:px-6">
            {children}
...
function ThreadMessage({ className, ...props }: ComponentProps<typeof MessageScroller.Item>) {
  return (
